...
{
    internal static class CC_SkinsLibrary
    {
        #region Constructor
        static CC_SkinsLibrary()
        {
            _Initialized = false;
        ...
        {
            if (List.ContainsKey(SkinID))
                return (GUISkin)CC_MBE.Instantiate(List[SkinID]);
            else
            {
                CC_MBE.LogFormatted("Unable to copy GUISkin to {0}, GUISkin not found", SkinID);
        ...
        internal static void AddStyle(ref GUISkin SkinToAction, GUIStyle NewStyle)
        {
            ...
            {
                CC_MBE.LogFormatted("No Name Provided in the Style to add to {0}. Cannot add this.", SkinToAction.name);
                return;
            }
